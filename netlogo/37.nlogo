...
to setup
  ...
  ask cats [set color brown set shape "cat" set lives 9 set label lives]
  create-dogs num-dogs
  ask dogs [set color grey set shape "dog" set age 1 set label age]
end

to wiggle
  ask turtles [rt random 31 fd 1 lt random 31]
end
to go
  ask dogs [
    if count cats-on neighbors > 0 [
    ask (one-of (cats-on neighbors)) [
      set lives (lives - 1)
      set label lives
      ask (one-of (dogs-on neighbors)) [
        set age (age + 1)
        if age >= 10 [die]
        set label age
      ]
